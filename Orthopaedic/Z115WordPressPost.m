...
- (NSString *)generateHtml:(int)fontSize
{
    if (!self.postHtml)
    {
        ...
        [html appendFormat:@"<html><head> <meta name=\"viewport\" content=\"width=device-width, user-scalable=no initial-scale=1.0\" /><script type=\"text/javascript\" src=\"z115wordpress.js\"></script></script><script type=\"text/javascript\">function changeFontSize(newSize) {document.getElementById('singlentry').style.fontSize = newSize + 'px';}  var anchor=0; function findAnchor() { anchor++; var arr = document.getElementsByClassName('orthopaedic_heading'); if(anchor > arr.length) anchor = 1; window.location = '#anchor' + anchor; }</script>"];
